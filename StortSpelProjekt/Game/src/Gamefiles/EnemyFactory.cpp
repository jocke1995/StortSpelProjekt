...
Entity* EnemyFactory::AddEnemy(std::string entityName, Model* model, int hp, float3 pos, std::wstring sound3D, unsigned int compFlags, unsigned int aiFlags, float scale, float3 rot, std::string aiTarget)
{
	for (auto pair : m_EnemyComps)
	{
	...
	return Add(entityName, model, hp, pos, sound3D, compFlags, aiFlags, enemy->dim, scale, rot, aiTarget);
}
...
Entity* EnemyFactory::AddExistingEnemy(std::string entityName, float3 pos)
{
	for (auto pair : m_EnemyComps)
	{
		...
		if (pair.first == entityName)
		{
			...
			return Add(name, enemy->model, enemy->hp, pos, enemy->sound3D, enemy->compFlags, enemy->aiFlags, enemy->dim, enemy->scale, enemy->rot, enemy->targetName);
		}
		else
		{
...
Entity* EnemyFactory::AddExistingEnemyWithChanges(std::string entityName, float3 pos, unsigned int compFlags, unsigned int aiFlags, float scale, float3 rot, int hp)
{
	for (auto pair : m_EnemyComps)
	{
		...
		if (pair.first == entityName)
		{
			...
			return Add(name, enemy->model, newHP, pos, enemy->sound3D, newCompFlags, newAiFlags, enemy->dim, newScale, newRot, enemy->targetName);
		}
		else
		{
...
Entity* EnemyFactory::Add(std::string name, Model* model, int hp, float3 pos, std::wstring sound3D, unsigned int compFlags, unsigned int aiFlags, float3 dim, float scale, float3 rot, std::string aiTarget)
{
